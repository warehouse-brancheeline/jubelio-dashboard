...
export function BusinessSummaryView({ controller, onOrders, onInventory }: Props) {
  ...
  const settlementCoverage = k.order_count > 0 ? k.settlement_data_count / k.order_count * 100 : 0;
  const settlementReliable = settlementCoverage >= 90;
  const syncHealth = summary.quality.sync_health;
  const funnelSelection: Record<string, OrderSelection> = {
    "Order masuk": {},
    "Diproses": { statuses: ["PROCESSING", "READY_TO_SHIP", "SHIPPED", "COMPLETED"] },
    "Siap dikirim": { statuses: ["READY_TO_SHIP", "SHIPPED", "COMPLETED"] },
    "Dikirim": { statuses: ["SHIPPED", "COMPLETED"] },
    "Selesai": { statuses: ["COMPLETED"] },
  ...
  return (
    <div className="business-summary">
      ...
      {syncHealth?.stale && (
        <div className="quality-alert"><AlertCircle size={18} /><span>Cache dashboard tertinggal {formatNumber(syncHealth.missing_from_cache)} order{syncHealth.lag_minutes === null ? "" : ` (${syncHealth.lag_minutes.toFixed(0)} menit)`}. Angka ditandai belum final sampai refresh cache berhasil.</span></div>
      )}
      ...
      <section className="metric-grid">
        ...
        <MetricCard label="Belum diproses" value={formatCompactNumber(k.new_order_count)} fullValue={formatNumber(k.new_order_count)} definition="Order aktif yang masih berada pada tahap baru atau siap diproses." hint={`${k.pending_rate.toFixed(1)}% dari order aktif, lihat detail`} delta={percentChange(k.new_order_count, c.new_order_count, comparisonReady)} icon={<Clock3 size={20} />} tone="red" onClick={() => onOrders({ statuses: ["NEW"] })} />
        <MetricCard label="Sedang diproses" value={formatCompactNumber(k.processing_order_count + k.ready_to_ship_count)} definition="Order pada tahap processing dan siap dikirim." hint={`${formatNumber(k.ready_to_ship_count)} siap dikirim, lihat detail`} delta={null} icon={<PackageCheck size={20} />} tone="blue" onClick={() => onOrders({ statuses: ["PROCESSING", "READY_TO_SHIP"] })} />
        <MetricCard label="Sudah dikirim" value={formatCompactNumber(k.shipped_order_count)} definition="Order sudah diserahkan ke pengiriman tetapi belum completed." hint="Buka order dalam perjalanan" delta={null} icon={<Truck size={20} />} tone="blue" onClick={() => onOrders({ statuses: ["SHIPPED"] })} />
        <MetricCard label="Order selesai" value={formatCompactNumber(k.completed_order_count)} definition="Order dengan kelompok status completed." hint={`${k.completion_rate.toFixed(1)}% completion rate, lihat detail`} delta={completedDelta} icon={<CheckCircle2 size={20} />} onClick={() => onOrders({ statuses: ["COMPLETED"] })} />
        <MetricCard label="Dibatalkan/retur" value={formatCompactNumber(k.cancelled_order_count + k.returned_order_count)} definition="Order cancelled dan returned, tidak dihitung sebagai nilai order valid." hint={`${k.cancellation_rate.toFixed(1)}% cancellation rate, lihat detail`} delta={cancelDelta} icon={<AlertCircle size={20} />} tone="red" onClick={() => onOrders({ statuses: ["CANCELLED", "RETURNED"] })} />
      </section>
      ...
      <section className="metric-grid compact">
        ...
        <MetricCard label="Revenue completed" value={formatCompactCurrency(k.completed_revenue)} fullValue={formatCurrency(k.completed_revenue)} definition="Total nilai order completed. Ini bukan nilai pencairan." hint={`${formatCompactCurrency(k.unfinished_value)} belum selesai, lihat detail`} delta={null} icon={<Banknote size={20} />} onClick={() => onOrders({ statuses: ["COMPLETED"] })} />
        <MetricCard label="Nilai sudah cair" value={settlementReliable ? formatCompactCurrency(k.settled_value) : "N/A"} fullValue={settlementReliable ? formatCurrency(k.settled_value) : `Cakupan data pencairan baru ${settlementCoverage.toFixed(1)}%`} definition="Nilai settlement/payout yang secara eksplisit ditandai cair oleh sumber." hint={settlementReliable ? `${formatNumber(k.settled_order_count)} order cair, lihat detail` : `Cakupan pencairan ${settlementCoverage.toFixed(1)}%, belum layak`} delta={settlementReliable ? percentChange(k.settled_value, c.settled_value, c.settlement_available) : null} icon={<Banknote size={20} />} tone="amber" onClick={() => onOrders({ settlementStatus: "SETTLED" })} />
      ...
      <section className="summary-two-columns">
        <article className="panel summary-panel"><SectionTitle eyebrow="FUNNEL ORDER" title="Pergerakan proses" />
          <div className="funnel-list">{summary.funnel.map((row, index) => { const previous = summary.funnel[index-1]?.order_count; const conversion = previous ? row.order_count / previous * 100 : 100; return <button key={row.stage} type="button" onClick={() => onOrders(funnelSelection[row.stage] ?? {})}><span>{row.stage}</span><strong>{formatNumber(row.order_count)}</strong><small>{index ? `${conversion.toFixed(1)}% dari tahap sebelumnya` : "Tahap awal"}</small><i style={{ width: `${Math.max(5, summary.funnel[0]?.order_count ? row.order_count/summary.funnel[0].order_count*100 : 0)}%` }}/></button>; })}</div>
        </article>
        <article className="panel summary-panel"><SectionTitle eyebrow="PENCAIRAN" title="Revenue dan settlement" />
          {settlementReliable ? <div className="settlement-summary"><div><span>Revenue completed</span><strong>{formatCurrency(k.completed_revenue)}</strong></div><div><span>Sudah cair</span><strong>{formatCurrency(k.settled_value)}</strong></div><div><span>Outstanding teridentifikasi</span><strong>{formatCurrency(k.unsettled_value)}</strong></div></div> : <Empty text={`Data pencairan baru mencakup ${settlementCoverage.toFixed(1)}% order. Nilai payout disembunyikan agar tidak menyesatkan.`} />}
        </article>
      </section>
